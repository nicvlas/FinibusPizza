...
public class NiveauAPCControleur implements Initializable {
	...
	/*
	 * Initialisation de tous les paramÃ¨tres de gestion d'une pizza Ã  zero
	 */
	...
	/*
	 * Ouverture le menu de préparation de la pÃ¢te
	 */
	...
	/*
	 * Remet Ã  zero les éléments de la commande choisie
	 */
	 ...
	 * @param unClient : sert Ã  savoir si le client est Karen
	 * @param _img : ImageView Ã  changer
	 */
	...
	/*
	 * Changement de client aprÃ¨s la vente d'une commande
	 ...
	 * @param lb : le label de l'ingrédient Ã  mettre
	 */
	...
	public String[] elementsIngredients(String element) {
		...
		if(retour.length != 3 && retour.length != 4) {
			throw new InternalError(element + " ne correspond pas Ã¯Â¿Â½ un String d'un fichier contenant des ingrédients traitables");
		}
		return retour;
	}
	/*
	...
	/*
	 * Remise Ã  zero la commande des clients
	 */
    ...
    public void afficherCommande(Commande c){
        ...
        for (Map.Entry m : c.getLesIngredients().entrySet()) {
            ...
            // } else if(tmp.compareTo("crÃ¨meBase") ==0){
            switch(tmp){
                case "tomatebase":
	...
	/**
	 * Pétrir la pÃ¢te. GénÃ¨re une pÃ¢te dans pateActuelle. Enregistre l'heure
	 * systÃ¨me du clic.
	 *
	 * @param event
	 * @author Nicolas
	...
	/**
	 * ArrÃªte le pétrissage. Compare l'heure systÃ¨me d'arrÃªt de pétrissage Ã 
	 * celle du commencement. Si >5 ou <5, on recommence petrir(). Si =5, on
	 * passe Ã  la pose des ingrédients
	 *
	 * @param event
	 * @throws IOException
	 * @author Nicolas
	...
	void stop(ActionEvent event) throws IOException {
		long end = System.currentTimeMillis(); // on prend l'heure systÃ¨me de
												// fin de tÃ¢che
	...
	void redirigerVictoire(String chemin, Integer score) throws IOException {
		...
		// Ouvre la fenÃªtre de la victoire
		...
		if (score > 0) {
			...
			if (niveauActuel.isTmp()) {
				controllerVictoire.btn_suivant_edit.setText("Ã‰diter");
